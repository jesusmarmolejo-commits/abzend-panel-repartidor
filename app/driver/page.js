...
export default function DriverPanel() {
  ...
  const submitPOD = async () => {
    ...
    if (!currentSignatureBlob) {
      currentSignatureBlob = await new Promise(resolve => canvas.toBlob(resolve))
    }
    ...
    try {
      ...
      const { error: sigError } = await supabase.storage
        .from('pod-media')
        .upload(sigPath, currentSignatureBlob, { contentType: 'image/png' })
      if (!sigError) {
      ...
      // Mostrar resumen con preview capturado ahora
      setPodSummary({
