...
const STATUS_COLOR = { pending:'0FAEEDA', assigned:'0eef2ff', picked_up:'0eef2ff', in_transit:'0EFF6FF', delivered:'0DCFCE7', cancelled:'0FEE2E2' }
const STATUS_TEXT  = { pending:'092400E', assigned:'0065F46', picked_up:'0065F46', in_transit:'01E40AF', delivered:'0166534', cancelled:'0991B1B' }
...
export default function AdminPanel() {
  ...
  // ── THEME ─────────────────────────────────────────────────
  const bg   = dark ? '0111827' : '0F9FAFB'
  const card = dark ? '01F2937' : '0FFFFFF'
  const text = dark ? '0F9FAFB' : '0111827'
  const sub  = dark ? '09CA3AF' : '06B7280'
  const bdr  = dark ? '0374151' : '0E5E7EB'
  const sbg  = dark ? '0111827' : '0FFFFFF'
  const sact = dark ? '0FFFFFF' : '0000000'
  const sinact = dark ? '06B7280' : '06B7280'

  if (loading) return (
    <div style={{display:'flex',alignItems:'center',justifyContent:'center',minHeight:'100vh',background:'04f46e5'}}>
      <div style={{background:'0fff',borderRadius:16,padding:'2rem',textAlign:'center'}}><p style={{color:'04f46e5',fontWeight:600}}>Cargando...</p></div>
    </div>
  )
  ...
  return (
    <div style={{display:'flex',minHeight:'100vh',background:bg,fontFamily:'system-ui,sans-serif',color:text}}>
      ...
      <div style={{width:220,background:sbg,borderRight:`1px solid ${bdr}`,display:'flex',flexDirection:'column',padding:'1.25rem 0',flexShrink:0}}>
        ...
        {navItems.map(n=>(
          <div key={n.id} onClick={()=>setSection(n.id)}
            style={{display:'flex',alignItems:'center',gap:10,padding:'10px 1.25rem',cursor:'pointer',fontSize:14,borderRadius:6,margin:'1px 8px',
              background:section===n.id?(dark?'0374151':'0000'):' transparent',
              color:section===n.id?'0fff':sinact,fontWeight:section===n.id?600:400}}>
        ...
        <div style={{marginTop:'auto',padding:'1rem 1.25rem',borderTop:`1px solid ${bdr}`}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            <div style={{width:32,height:32,borderRadius:'50%',background:'0374151',display:'flex',alignItems:'center',justifyContent:'center',color:'0fff',fontSize:13,fontWeight:700}}>A</div>
            <div><div style={{fontSize:13,fontWeight:600,color:text}}>Admin</div><div style={{fontSize:11,color:sub}}>{user?.email}</div></div>
          </div>
          <button onClick={logout} style={{display:'flex',alignItems:'center',gap:6,marginTop:'0.75rem',background:'none',border:'none',cursor:'pointer',color:'0EF4444',fontSize:13}}>
            ↩ Cerrar Sesión
          </button>
        </div>
      </div>
      ...
      <div style={{flex:1,padding:'2rem',overflow:'auto'}}>
        {msg && <div style={{position:'fixed',top:16,right:16,background:'04f46e5',color:'0fff',padding:'10px 18px',borderRadius:8,fontSize:13,zIndex:300}}>{msg}</div>}
              ...
              <div style={{display:'flex',alignItems:'flex-end',gap:10,height:140}}>
                {last7.map((d,i)=>(
                  <div key={i} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:4}}>
                    <div style={{width:'100%',background:dark?'0374151':'0E5E7EB',borderRadius:'4px 4px 0 0',height:120,display:'flex',alignItems:'flex-end'}}>
                      <div style={{width:'100%',background:dark?'0F9FAFB':'0111827',borderRadius:'4px 4px 0 0',height:`${(d.count/maxBar)*100}%`,minHeight:d.count?4:0,transition:'height .3s'}}/>
                    </div>
        ...
        {section==='orders' && (
          <div>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'1.5rem'}}>
              ...
              <button onClick={()=>router.push('/orders/new')} style={{padding:'10px 18px',background:'0111827',color:'0fff',border:'none',borderRadius:8,cursor:'pointer',fontSize:14,fontWeight:600}}>Nueva Orden</button>
            </div>
                    ...
                    <tr key={o.id} style={{borderBottom:`1px solid ${bdr}`}}>
                      ...
                      <td style={{padding:'10px 6px'}}>
                        <div style={{display:'flex',gap:4}}>
                          {o.status==='pending'&&<button onClick={()=>{setAssignOrder(o);setAssignDriver('')}} style={{padding:'4px 8px',background:'04f46e5',color:'0fff',border:'none',borderRadius:5,cursor:'pointer',fontSize:11}}>Asignar</button>}
                          <button onClick={()=>{setStatusOrder(o);setNewStatus(o.status);setStatusCode('')}} style={{padding:'4px 8px',background:'0185FA5',color:'0fff',border:'none',borderRadius:5,cursor:'pointer',fontSize:11}}>Estado</button>
                        </div>
                      </td>
                    </tr>
                  ))}
          ...
          <div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:20}}>
              <div style={{background:card,border:`1px solid ${bdr}`,borderRadius:10,padding:'1.5rem'}}>
                ...
                <button onClick={calcQuote} style={{width:'100%',padding:'11px',background:dark?'0F9FAFB':'0111827',color:dark?'0111827':'0fff',border:'none',borderRadius:8,cursor:'pointer',fontSize:14,fontWeight:600}}>
                  🧮 Calcular Tarifa
                </button>
              </div>
              <div style={{background:card,border:`1px solid ${bdr}`,borderRadius:10,padding:'1.5rem'}}>
                {!quoteResult ? (
                  ...
                  <div>
                    ...
                    {[
                      {label:'Estándar (3-5 días)', value:quoteResult.standard, color:'04f46e5'},
                      {label:'Express (1-2 días)', value:quoteResult.express, color:'0185FA5'},
                      {label:'Mismo día', value:quoteResult.same_day, color:'04f46e5'},
                    ].map((r,i)=>(
                    ...
                    <div style={{marginTop:'1rem',padding:'10px',background:dark?'0374151':'0F9FAFB',borderRadius:8,fontSize:12,color:sub}}>
                  ...
                  {clients.map(c=>(
                    <tr key={c.id} style={{borderBottom:`1px solid ${bdr}`}}>
                      ...
                      <td style={{padding:'12px 16px'}}>
                        <span style={{background:'0DCFCE7',color:'0166534',fontSize:11,padding:'3px 10px',borderRadius:20,fontWeight:600}}>Activo</span>
                      </td>
                      <td style={{padding:'12px 16px'}}>
                        <button onClick={()=>deleteClient(c.id)} style={{background:'none',border:'none',cursor:'pointer',color:'0EF4444',fontSize:16}}>🗑</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
                      ...
                      <td style={{padding:'12px 16px'}}>
                        <div style={{display:'flex',alignItems:'center',gap:8}}>
                          <div onClick={()=>toggleDriver(d.id,d.status)}
                            style={{width:42,height:24,borderRadius:12,background:d.status==='online'?'0111827':'0D1D5DB',cursor:'pointer',position:'relative',transition:'background .2s'}}>
                            <div style={{position:'absolute',top:2,left:d.status==='online'?18:2,width:20,height:20,borderRadius:'50%',background:'0fff',transition:'left .2s'}}/>
                          </div>
                          <span style={{fontSize:13,color:d.status==='online'?text:sub}}>{d.status==='online'?'Activo':'Inactivo'}</span>
                        </div>
                      </td>
                      <td style={{padding:'12px 16px'}}>
                        <button style={{background:'none',border:'none',cursor:'pointer',color:'0EF4444',fontSize:16}}>🗑</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
                  ...
                  {last7.map((d,i)=>{
                    ...
                    return (
                      <div key={i} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:4}}>
                        ...
                        <div style={{width:'100%',background:dark?'0374151':'0E5E7EB',borderRadius:'4px 4px 0 0',height:140,display:'flex',alignItems:'flex-end'}}>
                          <div style={{width:'100%',background:'04f46e5',borderRadius:'4px 4px 0 0',height:`${Math.max((rev/maxRev)*100,rev>0?4:0)}%`,transition:'height .3s'}}/>
                        </div>
        ...
        {assignOrder && (
          <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.4)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200}}>
            <div style={{background:card,borderRadius:14,padding:'1.5rem',width:'100%',maxWidth:420,margin:'1rem',border:`1px solid ${bdr}`}}>
              <h3 style={{fontWeight:600,marginBottom:6,color:text}}>Asignar 0{assignOrder.tracking_code}</h3>
              ...
              <div style={{display:'flex',gap:10,justifyContent:'flex-end'}}>
                ...
                <button onClick={doAssign} disabled={processing} style={{padding:'8px 18px',background:'04f46e5',color:'0fff',border:'none',borderRadius:8,cursor:'pointer',fontSize:13,fontWeight:600,opacity:processing?.6:1}}>
        ...
        {statusOrder && (
          <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.4)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200}}>
            <div style={{background:card,borderRadius:14,padding:'1.5rem',width:'100%',maxWidth:420,margin:'1rem',border:`1px solid ${bdr}`}}>
              <h3 style={{fontWeight:600,marginBottom:6,color:text}}>Cambiar estado 0{statusOrder.tracking_code}</h3>
              <div style={{marginBottom:'1rem'}}>
              ...
              <div style={{display:'flex',gap:10,justifyContent:'flex-end'}}>
                ...
                <button onClick={doStatus} disabled={processing} style={{padding:'8px 18px',background:'0185FA5',color:'0fff',border:'none',borderRadius:8,cursor:'pointer',fontSize:13,fontWeight:600,opacity:processing?.6:1}}>
    ...
    script.onload = () => {
      ...
      active.forEach((d, i) => {
        ...
        const icon = L.divIcon({
          html: `<div style="background:04f46e5;color:0fff;padding:4px 8px;border-radius:20px;font-size:11px;white-space:nowrap;font-weight:600;box-shadow:0 2px 6px rgba(0,0,0,0.3)">🚚 ${d.user?.full_name?.split(' ')[0]||'Driver'}</div>`,
...
function DonutChart({ orders, dark, sub }) {
  ...
  const slices = Object.entries(counts).map(([status, count]) => ({
    ...
    color: {pending:'0F59E0B',assigned:'04f46e5',picked_up:'01D9E75',in_transit:'0185FA5',delivered:'010B981',cancelled:'0EF4444'}[status]||'09CA3AF',
